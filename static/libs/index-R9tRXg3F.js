import { L as LRLanguage, a as LanguageSupport, i as indentNodeProp, c as continuedIndent, f as foldNodeProp, s as styleTags, t as tags, E as ExternalTokenizer, b as ifNotIn, d as completeFromList, e as LRParser, g as syntaxTree } from './markdown-CHk9_O5A.js';
...
function isAlpha(ch) {
  return ch >= 65 /* Ch.A */ && ch <= 90 /* Ch.Z */ ||
    ch >= 97 /* Ch.a */ && ch <= 122 /* Ch.z */ ||
    ch >= 48 /* Ch._0 */ && ch <= 57 /* Ch._9 */;
}
function isHexDigit(ch) {
  return ch >= 48 /* Ch._0 */ && ch <= 57 /* Ch._9 */ ||
...
function readLiteral(input, endQuote, backslashEscapes) {
  for (let escaped = false;;) {
    ...
    escaped = backslashEscapes && !escaped &&
      input.next == 92 /* Ch.Backslash */;
    input.advance();
  }
}
...
function readPLSQLQuotedLiteral(input, openDelim) {
  ...
  let closeDelim = matchingDelim < 0
    ? openDelim
    : "]}>)".charCodeAt(matchingDelim);
  for (;;) {
    if (input.next < 0) {
      return;
    }
...
function readWordOrQuoted(input) {
  if (
    ...
    input.next == 34 /* Ch.DoubleQuote */ || input.next == 96 /* Ch.Backtick */
  ) {
...
function tokensFor(d) {
  return new ExternalTokenizer((input) => {
    ...
    if (inString(next, Space)) {
      ...
      next == 47 /* Ch.Slash */ && input.next == 47 /* Ch.Slash */ &&
        d.slashComments
    ) {
      eol(input);
      input.acceptToken(LineComment);
    } else if (
      next == 45 /* Ch.Dash */ && input.next == 45 /* Ch.Dash */ &&
      ...
      const endQuote = next == 91 /* Ch.BracketL */
        ? 93 /* Ch.BracketR */
        : next;
      ...
      d.unquotedBitLiterals && next == 48 /* Ch._0 */ &&
      input.next == 98 /* Ch.b */
    ) {
      input.advance();
      readBits(input);
      ...
      next == 48 /* Ch._0 */ &&
        (input.next == 120 /* Ch.x */ || input.next == 88 /* Ch.X */) ||
      (next == 120 /* Ch.x */ || next == 88 /* Ch.X */) &&
        input.next == 39 /* Ch.SingleQuote */
    ) {
      ...
      next == 46 /* Ch.Dot */ && input.next >= 48 /* Ch._0 */ &&
      input.next <= 57 /* Ch._9 */
    ) {
      ...
      input.acceptToken(
        input.next == 46 /* Ch.Dot */ ||
          input.peek(-word.length - 1) == 46 /* Ch.Dot */
          ? Identifier
...
const parser$1 = /*@__PURE__*/ LRParser.deserialize({
  ...
  goto:
    "#hwPPPPPPPPPPPPPPPPPPPPPPPPPPx||||!Y!^!d!xPPP#[TYOZeUORSTWZbdfqT[OZQZORiZSWOZQbRQdSQfTZgWbdfqQ^PWk^lmrQl_Qm`RrseVORSTWZbdfq",
  nodeNames:
...
function sourceContext(state, startPos) {
  ...
  if (
    pos.name == "Identifier" || pos.name == "QuotedIdentifier" ||
    pos.name == "Keyword"
  ) {
    return {
      from: pos.from,
      quoted: pos.name == "QuotedIdentifier"
...
const EndFrom = /*@__PURE__*/ new Set(
  /*@__PURE__*/ "where group having order union intersect except all distinct limit offset fetch for"
    .split(" "),
);
...
class CompletionLevel {
  ...
  child(name) {
    ...
    if (name && !this.list.some((c) => c.label == name)) {
      this.list.push(
        nameCompletion(name, "type", this.idQuote, this.idCaseInsensitive),
      );
    }
    return (children[name] = new CompletionLevel(
      this.idQuote,
      this.idCaseInsensitive,
    ));
  }
  maybeChild(name) {
  ...
  addNamespaceObject(namespace) {
    for (let name of Object.keys(namespace)) {
      ...
      let parts = name.replace(/\\?\./g, (p) => p == "." ? "\0" : p).split(
        "\0",
      );
      let scope = this;
...
function nameCompletion(label, type, idQuote, idCaseInsensitive) {
  if (
    (new RegExp("^[a-z_][a-z_\\d]*$", idCaseInsensitive ? "i" : "")).test(label)
  ) {
    return { label, type };
  }
...
function completeFromSchema(
  ...
  var _a;
  let idQuote =
    ((_a = dialect === null || dialect === void 0
            ? void 0
            : dialect.spec.identifierQuotes) === null || _a === void 0
      ? void 0
      : _a[0]) || '"';
  let top = new CompletionLevel(
    idQuote,
    !!(dialect === null || dialect === void 0
      ? void 0
