import { e as LRParser, E as ExternalTokenizer, C as ContextTracker, s as styleTags, t as tags, a as LanguageSupport, r as defineCSSCompletionSource, L as LRLanguage, f as foldNodeProp, l as foldInside, i as indentNodeProp, c as continuedIndent } from './markdown-Cvm-Jczl.js';
...
const spaces = new ExternalTokenizer((input, stack) => {
  if (stack.dialectEnabled(Dialect_indented)) {
    ...
    if (input.next < 0 && stack.canShift(eof)) {
      ...
      ((prev = input.peek(-1)) == newlineChar || prev < 0) &&
...
const indentedMixins = new ExternalTokenizer((input, stack) => {
  if (
    (input.next == plus || input.next == equals) &&
    stack.dialectEnabled(Dialect_indented)
  ) {
    input.acceptToken(
      input.next == equals ? IndentedMixin : IndentedInclude,
      1,
    );
  }
});
...
const identifiers = new ExternalTokenizer((input, stack) => {
  for (let inside = false, dashes = 0, i = 0;; i++) {
    let { next } = input;
    if (
      isAlpha(next) || next == dash || next == underscore ||
      (inside && isDigit(next))
    ) {
      ...
      if (inside) {
        input.acceptToken(
          dashes == 2 && stack.canShift(VariableName)
            ? VariableName
            : stack.canShift(queryIdentifier)
            ? queryIdentifier
            : next == parenL
            ? callee
            : identifier,
...
const interpolationEnd = new ExternalTokenizer((input) => {
  if (input.next == braceR) {
    input.advance();
    while (
      isAlpha(input.next) || input.next == dash || input.next == underscore ||
      isDigit(input.next)
    ) {
      input.advance();
    }
...
const descendant = new ExternalTokenizer((input) => {
  if (space.includes(input.peek(-1))) {
    let { next } = input;
    if (
      isAlpha(next) || next == underscore || next == hash || next == period ||
...
function IndentLevel(parent, depth) {
  this.parent = parent;
  this.depth = depth;
  this.hash = (parent ? parent.hash + parent.hash << 8 : 0) + depth +
    (depth << 4);
}
...
const parser = LRParser.deserialize({
  ...
  dialects: { indented: 0 },
  specialized: [
    { term: 172, get: (value) => spec_identifier[value] || -1 },
    { term: 171, get: (value) => spec_callee[value] || -1 },
    { term: 80, get: (value) => spec_AtKeyword[value] || -1 },
    {
      term: 173,
    ...
    props: [
      /*@__PURE__*/ foldNodeProp.add({
        ...
        Comment(node, state) {
          return {
            from: node.from + 2,
            to: state.sliceDoc(node.to - 2, node.to) == "*/"
              ? node.to - 2
              : node.to,
          };
        },
...
source.
*/
const sassCompletionSource = /*@__PURE__*/ defineCSSCompletionSource((node) =>
  node.name == "VariableName" || node.name == "SassVariableName"
);
/**
...
function sass(config) {
  return new LanguageSupport(
    (config === null || config === void 0 ? void 0 : config.indented)
      ? indentedSassLanguage
      : sassLanguage,
